...
if exist \Apps\CDSS\bin\TSTool.bat goto appsconfig
goto defaultconfig
:appsconfig
SET HOMED=\Apps\CDSS
goto homedone
...
:homedone
rem Should be relative to the CDSS home directory...
SET JREHOMED=%HOMED%\jre_142

rem Run the Java Runtime Environment (JRE), which runs the StateView software.
...
"%JREHOMED%\bin\java" -Xmx256m -cp "%HOMED%\bin\CWRAT_142.jar;%HOMED%\bin\HydroBaseDMI_142.jar;%HOMED%\bin\msbase.jar;%HOMED%\bin\mssqlserver.jar;%HOMED%\bin\msutil.jar;%HOMED%\bin\SatmonSysDMI_142.jar;%HOMED%\bin\RTi_Common_142.jar;TS_Services_142.jar" DWR.DMI.CWRAT.CWRAT -home "%HOMED%" -viewonly true %1 %2 %3 %4 %5 %6 %7 %8 %9
