...
#  install file:"StateView_version_setup.exe"
#  This .exe install file will setup StateView
...
#  StateView - installs StateView specific files (jars, bats, etc)
#  Start Menu Shortcuts - installs shortcuts to run StateView
#                          and uninstall StateView
...
OutFile "StateView_${VERSION}_Setup.exe"
InstallDir "C:\CDSS"
...
#   choseStateView - Used for dependencies in later sections
#                0 = user chose not to install StateView,
#                1 = user chose to install StateView
#  
# BRIEF:
...
#
# BRIEF: 
#  Installs the StateView specific files.
...
#
##################################################
Section "StateView" StateView

    # set choseStateView variable to true since it was chosen
    strcpy $choseStateView "1"
    ...
    File ..\..\externals\CWRAT\CWRAT_142.jar
    File ..\..\scripts\StateView.bat
    
    SetOutPath $INSTDIR\system
    File ..\..\graphics\stateview.gif
    
    # Insert the -home Directory into the .bat file
    # according to the user's install location
    ${textreplace::ReplaceInFile} "$INSTDIR\bin\StateView.bat" "$INSTDIR\bin\StateView.bat" "SET HOMED=\CDSS" "SET HOMED=$INSTDIR" "" $0
    ...
    ${textreplace::ReplaceInFile} "$INSTDIR\bin\StateView.ini" \
    "$INSTDIR\bin\StateView.ini" "-home C:\CDSS" "-home $INSTDIR" "" $0
    ...
    WriteRegStr HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\$(^Name)" DisplayIcon $INSTDIR\uninstall\$(^Name).exe
...
#  for StateView to C:\CDSS\doc\StateView
#
...
Section "Documentation" Docs
    ...
    # copy documentation
    SetOutPath $INSTDIR\doc\StateView
    SetOverwrite on
    File /r /x *svn* ..\..\doc\StateView\*
...
#  shortcuts as CDSS -> StateView -> uninstall
#                             -> run stateview
#  
##############################################
Section "Start Menu" StartMenu

    # make sure user chose to install StateView
    strcmp $choseStateView "0" 0 +2
      Goto skipMenu
    ...
    CreateShortCut "$SMPROGRAMS\$StartMenuGroup\StateView.lnk" "$INSTDIR\bin\StateView.exe"
    ...
    # Shortcut for StateView documentation
...
#  
############################################
Section /o "DesktopShortcut" DesktopShortcut
    ...
    SetOutPath $INSTDIR\bin
    CreateShortCut "$DESKTOP\StateView.lnk" "$INSTDIR\bin\StateView.exe"
    #CreateShortCut "$DESKTOP\StateView.lnk" "$INSTDIR\jre_142\bin\javaw.exe" "-Xmx256m -cp $\"CWRAT_142.jar;HydroBaseDMI_142.jar;mssqlall.jar;SatmonSysDMI_142.jar;RTi_Common_142.jar$\" DWR.DMI.CWRAT.CWRAT -home ..\ -viewonly true" "C:\CDSS\graphics\waterMark.bmp"
...
Section Uninstall
    ...
    Delete /REBOOTOK $INSTDIR\bin\StateView.bat
    Delete /REBOOTOK $INSTDIR\bin\StateView_RTi.bat
    #Delete /REBOOTOK $INSTDIR\bin\CWRAT_142.jar ; don't uninstall since CWRAT might still depend on it
    RmDir /r /REBOOTOK $INSTDIR\doc\StateView
    Delete /REBOOTOK $INSTDIR\system\stateview.gif
    Delete /REBOOTOK $INSTDIR\StateView_README.txt
    RmDir /r /REBOOTOK $INSTDIR\doc\StateView
    RmDir /REBOOTOK $INSTDIR\doc
    Delete /REBOOTOK $INSTDIR\bin\StateView.exe
    Delete /REBOOTOK $INSTDIR\bin\StateView.ini
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${Docs} "Enabling this component will install documentation to the CDSS\doc\StateView folder."
  !insertmacro MUI_DESCRIPTION_TEXT ${StateView} "Enabling this component will install all StateView software in the CDSS\bin folder."
  !insertmacro MUI_DESCRIPTION_TEXT ${StartMenu} "Enabling this component will install start menu folders and icons."
  !insertmacro MUI_DESCRIPTION_TEXT ${DesktopShortcut} "Enabling this component will install a desktop shortcut to run the StateView application."
...
Function .onInstSuccess
    ...
    # if 0 then user didn't choose to install StateView
    strcmp $choseStateView "0" 0 +2
      Goto skipThis
    
    # Run NativeJ to create the .exe file
    ExecWait '"$INSTDIR\bin\nativejc.exe" $INSTDIR\bin\StateView.njp'
    
    MessageBox MB_YESNO "Would you like to run the program?" IDYES true IDNO false
    true:
      Exec '"$INSTDIR\bin\StateView.exe"'
      #Exec '"$INSTDIR\jre_142\bin\javaw.exe" -Xmx256m -cp $\"CWRAT_142.jar;HydroBaseDMI_142.jar;mssqlall.jar;SatmonSysDMI_142.jar;RTi_Common_142.jar$\" DWR.DMI.CWRAT.CWRAT -home ..\ -viewonly true'
      Goto next
    false:
      DetailPrint "User chose to not start application"
    next:
    
    ##### Comment out later to include README.txt #####
    #MessageBox MB_YESNO "Would you like to view the README?" IDYES yes IDNO no
    #yes:
    #  Exec 'notepad.exe $INSTDIR\StateView_README.txt'
    #  Goto next2
    #no:
